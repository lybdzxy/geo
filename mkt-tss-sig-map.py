...
for i, ssp in enumerate(ssp_values):
    for j, index in enumerate(index_values):
        ...
        # 绘制降水数据的轮廓填充
        cf = ax.pcolormesh(lon, lat, trend.T, cmap=custom_cmap, shading='auto')
...
cbar = ColorbarBase(ax=axgr.cbar_axes[0], cmap=custom_cmap, ticks=[-1, 0, 1], orientation='vertical')
cbar.ax.set_yticklabels(labels)  # 设置标签
