...
def process_data(ssp_num, mod, year):
    input_path = f'E:/GEO/down/qpm/downed_{mod}_{ssp_num}_{year}.nc'
    ...
    prcp.to_netcdf(f'E:/GEO/etccdi/qpm/prcptot_{mod}_{ssp_num}_{year}.nc')
    ...
    r95p.to_netcdf(f'E:/GEO/etccdi/qpm/r95p_{mod}_{ssp_num}_{year}.nc')
    ...
    r99p.to_netcdf(f'E:/GEO/etccdi/qpm/r99p_{mod}_{ssp_num}_{year}.nc')
    # 计算 r95ptot,r99ptot
    r95ptot = r95p / prcp
    r95ptot.to_netcdf(f'E:/GEO/etccdi/qpm/r95ptot_{mod}_{ssp_num}_{year}.nc')
    r99ptot = r99p / prcp
    r99ptot.to_netcdf(f'E:/GEO/etccdi/qpm/r99ptot_{mod}_{ssp_num}_{year}.nc')
    # 计算 SDII（平均降水日降水量）
    sdii = indices.daily_pr_intensity(pr)
    sdii.to_netcdf(f'E:/GEO/etccdi/qpm/sdii_{mod}_{ssp_num}_{year}.nc')
    # 计算 Rx1day（连续 1 天内的最大日降水量）
    rx1day = indices.max_n_day_precipitation_amount(pr, window=1)
    rx1day.to_netcdf(f'E:/GEO/etccdi/qpm/rx1day_{mod}_{ssp_num}_{year}.nc')
    ...
    r10mm.to_netcdf(f'E:/GEO/etccdi/qpm/r10mm_{mod}_{ssp_num}_{year}.nc')
    # 计算 R20mm（连续 1 天中 20 mm 以上降水的次数）
    r20mm = ((pr > 20).groupby('time.dayofyear').sum(dim='time') > 0).sum(dim='dayofyear')
    #days_over_thresh = indices.days_over_precip_thresh(precip, thresh=20)
    r20mm.to_netcdf(f'E:/GEO/etccdi/qpm/r20mm_{mod}_{ssp_num}_{year}.nc')

def main():
    total_tasks = len(ssps) * len(names) * (2069 - 2015)
